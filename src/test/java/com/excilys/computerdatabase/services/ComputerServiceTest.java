...
public class ComputerServiceTest extends junit.framework.TestCase {
    ...
    // company1 = Company.getBuilder().name("Apple Inc.").id(1).build();
    // company2 = Company.getBuilder().name("Thinking Machines").id(2).build();
    //
    // computer1 =
    // Computer.getBuilder().name("CM-5").company(company1).id(1).introduced(dateTime)
    // .discontinued(dateTime2).build();
    // computer2 = Computer.getBuilder().name("MacBook
    // Pro").company(company2).id(2).build();
